...
`0.1.11` 支援窗簾／百葉的 `Close`（DPT 1.008 上下）、`Step`（DPT 1.007 步進/停止）、`POS`（DPT 5.001 絕對位置）標籤，與既有的 `Up/Down`／`Stop/Step`／`Position` 命名並存，合併為單一窗簾實體；DPT 不符的標籤（例如脈衝繼電器式的 `Close`/`Open` DPT 1.001）仍不會被誤判為窗簾。已匯入的工程檔會在升級後自動重新配對，不需手動重新上傳。
